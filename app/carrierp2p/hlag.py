    ...
    leg_list: list = [
        Leg.model_construct(
            pointFrom=PointBase.model_construct(
                locationName=leg["departure"]["location"]["locationName"],
            ...
            pointTo=PointBase.model_construct(
                locationName=leg["arrival"]["location"]["locationName"],
            ...
            transportations=Transportation.model_construct(
                transportType=str(leg.get("modeOfTransport", "Vessel")).title(),
                transportName=(
                    leg["vesselName"]
                    if (vessel_imo := leg.get("vesselIMONumber"))
                    else None
                ),
                referenceType="IMO" if vessel_imo and vessel_imo != "0000000" else None,
                reference=(
                    vessel_imo if vessel_imo and vessel_imo != "0000000" else None
                ),
            ),
            services=(
                Service.model_construct(
                    serviceCode=check_service_code,
                    serviceName=leg.get("carrierServiceName"),
                )
                if (check_service_code := leg.get("carrierServiceCode"))
                else None
            ),
            voyages=Voyage.model_construct(
                internalVoyage=(
                    internal_voy
                    if (internal_voy := leg.get("universalExportVoyageReference"))
                    else None
                )
            ),
        )
...
def process_schedule_data(
    task: dict, service: str, tsp: str, vessel_imo: str
) -> Iterator:
    ...
    transit_time: int = task.get(
        "transitTime",
        (
            datetime.fromisoformat(last_eta[:10]) - datetime.fromisoformat(first_etd[:10])
        ).days,
    )
    check_service_code: bool = (
        any(
            loop_code
            for loop_code in task["legs"]
            if loop_code.get("carrierServiceCode") == service
        )
        if service
        else True
    )
    ...
    check_vessel_imo: bool = (
        any(imo for imo in task["legs"] if imo.get("vesselIMONumber") == vessel_imo)
        if vessel_imo
        else True
    )
    transshipment_port: bool = (
        any(
            tsport["departure"]["location"]["UNLocationCode"] == tsp
            for tsport in task["legs"]
        )
        if check_transshipment and tsp
        else False
    )
    if (
        (transshipment_port or not tsp) and (check_service_code or not service) and check_vessel_imo
    ):
...
async def get_hlag_p2p(
    ...
    start_day: str = (
        etd.strftime("%Y-%m-%dT%H:%M:%S.%SZ")
        if etd
        else eta.strftime("%Y-%m-%dT%H:%M:%S.%SZ")
    )
    end_day: str = (
        (etd + timedelta(days=search_range)).strftime("%Y-%m-%dT%H:%M:%S.%SZ")
        if etd
        else (eta + timedelta(days=search_range)).strftime("%Y-%m-%dT%H:%M:%S.%SZ")
    )
    params: dict = {"placeOfReceipt": pol, "placeOfDelivery": pod}
    (
        params.update(
            {"departureDateTime:gte": start_day, "departureDateTime:lte": end_day}
        )
        if etd
        else params.update(
    ...
    def generate_schedule(data):
        return (
            ...
            for result in process_schedule_data(
                task=task, service=service, tsp=tsp, vessel_imo=vessel_imo
            )
        )
    response_cache = await db.get(
        ...
        log_component="hlcu original response file",
    )
    if response_cache:
    ...
    response_json = await anext(
        client.parse(scac="hlag", method="GET", url=url, params=params, headers=headers)
    )
    if response_json:
        ...
        background_task.add_task(
            ...
            params=params,
            value=response_json,
            log_component="hlag original response file",
        )
        return p2p_schedule
