...
sem = asyncio.Semaphore(40)#Control the number of thread by 20 as we have mutiple apps using the same API end point.it will easily exceeded the given API rate limit
        ...
        for p2p_response in gather_tasks:
            if p2p_response.status_code in (200, 206):
                ...
                for schedule in response['schedules']:
                    ...
                    result = orjson.dumps({'scac': scac, 'pol': first_pol, 'pod': last_pod, 'tsp': ';'.join(map(str, tsp)), 'service_codes': ';'.join(map(str, service_name if scac in ('MAEU', 'SEAU', 'SEJJ', 'MCPU', 'MAEI','MSCU') else service_code))})
                    if result in seen:
