...
class ClientSideCache:
    ...
    def generate_cache_key(self,key: Optional[uuid.UUID]=None,original_response:Optional[bool] = False,scac:Optional[str] = None,params:Optional[str] = None):
    ...
    async def set(self,  value: JSONResponse,expire:int = timedelta(hours = load_yaml()['data']['backgroundTasks']['scheduleExpiry']),
        ...
        async with self._pool.pipeline(transaction=True) as pipe:
            try:
                await pipe.watch(generate_key.urn) #tells Redis to monitor this key. If this key is modified by any other client before the transaction is executed, the transaction will be aborted.
                ...
                redis_set = pipe.set(name=generate_key.urn, value=orjson.dumps(value),ex=expire,nx=True) #Using nx = True to make sure the record doesnt exist and prevent us from overwriting the original record
