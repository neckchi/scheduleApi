...
def process_leg_data(leg_task:list)->list:
    leg_list: list = [Leg.model_construct(
            ...
            cutoffs=Cutoff.model_construct(docCutoffDate=si_cutoff,cyCutoffDate=next((led['CallDateTime'] for led in leg['Calls'][0]['CallDates'] if led.get('CallDateTime') and led['Type'] == 'CYCUTOFF'), None),
            ...
            transportations=Transportation.model_construct(transportType='Vessel', transportName= leg.get('TransportationMeansName'),
            ...
            voyages=Voyage.model_construct(internalVoyage=leg['Voyages'][0]['Description'] if leg.get('Voyages') else None)) for leg in leg_task]
    return leg_list
...
def process_schedule_data(task: dict, direct_only:bool |None,vessel_imo: str, service: str, tsp: str) -> Iterator:
    ...
    if (transshipment_port or not tsp) and (direct_only is None or check_transshipment != direct_only) and check_service_code and check_vessel_imo:
        ...
        schedule_body = Schedule.model_construct(scac='MSCU',pointFrom= first_point_from,pointTo= last_point_to, etd= first_etd,eta=last_eta,transitTime=transit_time,
                                                                            transshipment=check_transshipment,legs=process_leg_data(leg_task=task['Schedules']))
        yield schedule_body
...
async def get_msc_token(client:HTTPClientWrapper,background_task:BackgroundTasks, oauth: str, aud: str, rsa:str, msc_client: str, msc_scope: str, msc_thumbprint: str) ->AsyncIterator:
    ...
    response_token:dict = await anext(client.parse(method='POST',background_tasks=background_task,url=oauth, headers=headers, data=params,namespace='msc token',expire = timedelta(minutes=50)))
...
async def get_msc_p2p(client:HTTPClientWrapper, background_task:BackgroundTasks,url: str, oauth: str, aud: str, pw: str, msc_client: str, msc_scope: str,msc_thumbprint: str, pol: str, pod: str,
    ...
    response_json:dict = await anext(client.parse(method='GET', background_tasks=background_task,url=url, params=params, headers=headers,namespace='msc original response'))
    if response_json:
