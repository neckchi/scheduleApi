...
def process_leg_data(
    ...
    first_service_name: str | None = (
        schedule_task["serviceName"] if schedule_task["serviceName"] != "" else None
    )
    first_cy_cutoff: str | None = (
        schedule_task["terminalCutoff"]
        if schedule_task["terminalCutoff"] != ""
        else None
    )
    first_doc_cutoff: str | None = (
        schedule_task["docCutoff"] if schedule_task["docCutoff"] != "" else None
    )
    first_vgm_cutoff: str | None = (
        schedule_task["vgmCutoff"] if schedule_task["vgmCutoff"] != "" else None
    )
    if check_transshipment:
        leg_list: list = [
            Leg.model_construct(
                pointFrom=PointBase.model_construct(
                    locationCode=leg["departureUnloc"],
                    terminalName=leg["departureTerminal"],
                ),
                pointTo=PointBase.model_construct(
                    locationCode=leg["arrivalUnloc"],
                    terminalName=leg["arrivalTerminal"],
                ),
                ...
                cutoffs=(
                    ...
                    if index == 0 and (first_cy_cutoff or first_doc_cutoff or first_vgm_cutoff)
                    else None
                ),
                transportations=Transportation.model_construct(
                    ...
                    referenceType=(
                        None
                        if (transport_type := leg.get("transportID")) == "UNKNOWN"
                        else "IMO"
                    ),
                ...
                services=(
                    Service.model_construct(
                        serviceCode=service_code, serviceName=leg["serviceName"]
                    )
                    if (service_code := leg["serviceCode"]) or (leg["serviceName"] and leg["serviceName"] != "")
                    else None
                ),
                voyages=Voyage.model_construct(
                    internalVoyage=(
                        voyage_num
                        if (voyage_num := leg.get("conveyanceNumber"))
                        else None
                    )
                ),
            )
            for index, leg in enumerate(schedule_task["legs"])
        ]
    else:
        leg_list: list = [
            Leg.model_construct(
                pointFrom=PointBase.model_construct(
                    locationCode=first_point_from, terminalName=first_origin_terminal
                ),
                pointTo=PointBase.model_construct(
                    locationCode=last_point_to, terminalName=last_destination_terminal
                ),
                etd=first_etd,
                eta=last_eta,
                ...
                cutoffs=(
                    ...
                    if first_cy_cutoff or first_doc_cutoff or first_vgm_cutoff
                    else None
                ),
                transportations=Transportation.model_construct(
                    transportType="Vessel",
                ...
                services=(
                    Service.model_construct(
                        serviceCode=first_service_code, serviceName=first_service_name
                    )
                    if first_service_code or first_service_name
                    else None
                ),
                voyages=Voyage.model_construct(
                    internalVoyage=first_voyage if first_voyage else None
                ),
            )
        ]
    return leg_list
...
def process_response_data(
    ...
    check_service: bool = (
        service_code == service or service_name == service if service else True
    )
    check_transshipment: bool = len(task["legs"]) > 1
    transshipment_port: bool = (
        any(tsport["departureUnloc"] == tsp for tsport in task["legs"][1:])
        if check_transshipment and tsp
        else False
    )
    check_vessel_imo: bool = (
        (
            task["imoNumber"] == vessel_imo or any(imo for imo in task["legs"] if imo.get("transportID") == vessel_imo)
        )
        if vessel_imo
        else True
    )
...
async def get_one_access_token(
    ...
    if response_token is None:
        ...
        response_token: dict = await anext(
            client.parse(
                ...
                url=url,
                headers=headers,
                token_key=one_token_key,
                expire=timedelta(minutes=55),
            )
        )
    return response_token["access_token"]


async def get_one_p2p(
    client: HTTPClientWrapper,
    background_task: BackgroundTasks,
    url: str,
    turl: str,
    ...
    response_cache = await db.get(
        ...
        log_component="one original response file",
    )

    def generate_schedule(data):
        return (
            schedule_result
            for schedule_type in data
            for task in data[schedule_type]
            for schedule_result in process_response_data(
    ...
    token: str = await get_one_access_token(
        client=client, background_task=background_task, url=turl, auth=auth, api_key=pw
    )
    headers: dict = {
        "apikey": pw,
    ...
    response_json: dict = await anext(
        client.parse(scac="one", method="GET", url=url, params=params, headers=headers)
    )
