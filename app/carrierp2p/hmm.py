...
def process_leg_data(
    schedule_task: dict, last_point_to: str, check_transshipment: str | None
) -> list:
        ...
        [
            Leg.model_construct(
                pointFrom=PointBase.model_construct(
                    locationName=schedule_task["outboundInland"]["fromLocationName"],
                ...
                pointTo=PointBase.model_construct(
                    locationName=schedule_task["outboundInland"]["toLocationName"],
                ...
                etd=(
                    outbound_etd := schedule_task["outboundInland"][
                        "fromLocationDepatureDate"
                    ]
                ),
                eta=(
                    outbound_eta := schedule_task["outboundInland"][
                        "toLocationArrivalDate"
                    ]
                ),
                transitTime=int(
                    (
                        datetime.datetime.fromisoformat(outbound_eta) - datetime.datetime.fromisoformat(outbound_etd)
                    ).days
                ),
                transportations=Transportation.model_construct(
                    transportType=schedule_task["outboundInland"]["transMode"]
                ),
                voyages=Voyage.model_construct(internalVoyage="001"),
            )
        ]
        if schedule_task.get("outboundInland")
        else []
    )
    # main routing
    leg_list += [
        Leg.model_construct(
            pointFrom=PointBase.model_construct(
                locationName=legs["loadPort"],
                locationCode=legs["loadPortCode"],
                terminalName=(
                    first_pol_terminal_name
                    if legs["vesselSequence"] == 1
                    else first_pot_terminal_name
                ),
                terminalCode=(
                    first_pol_terminal_code
                    if legs["vesselSequence"] == 1
                    else first_pot_terminal_code
                ),
            ),
            pointTo=PointBase.model_construct(
                locationName=legs["dischargePort"],
                locationCode=legs.get("dischargePortCode") or last_point_to,
                terminalName=(
                    first_pot_terminal_name
                    if check_transshipment and legs["vesselSequence"] == 1
                    else last_pod_terminal_name
                ),
                terminalCode=(
                    first_pot_terminal_code
                    if check_transshipment and legs["vesselSequence"] == 1
                    else last_pod_terminal_code
                ),
            ),
            etd=etd,
            ...
            cutoffs=(
                ...
                if index == 0 and (first_cy_cutoff or first_doc_cutoff)
                else None
            ),
            transitTime=int(
                (
                    datetime.datetime.fromisoformat(eta) - datetime.datetime.fromisoformat(etd)
                ).days
            ),
            transportations=Transportation.model_construct(
                transportType=(
                    "Vessel" if (vessel_name := legs.get("vesselName")) else "Feeder"
                ),
                transportName=vessel_name,
                referenceType=(
                    "IMO" if (imo_code := legs.get("lloydRegisterNo")) else None
                ),
                reference=imo_code,
            ),
            services=(
                Service.model_construct(serviceCode=check_service)
                if (check_service := legs.get("vesselLoop"))
                else None
            ),
            voyages=Voyage.model_construct(
                internalVoyage=(
                    internal_voy if (internal_voy := legs.get("voyageNumber")) else None
                )
            ),
        )
        for index, legs in enumerate(schedule_task["vessel"])
        if (etd := legs.get("vesselDepartureDate"))
    ]
    # inbound
    leg_list += (
        [
            Leg.model_construct(
                pointFrom=PointBase.model_construct(
                    locationName=schedule_task["inboundInland"]["fromLocationName"],
                ...
                pointTo=PointBase.model_construct(
                    locationName=schedule_task["inboundInland"]["toLocationName"],
                ...
                etd=(
                    inbound_etd := schedule_task["inboundInland"][
                        "fromLocationDepatureDate"
                    ]
                ),
                eta=(
                    inbound_eta := schedule_task["inboundInland"][
                        "toLocationArrivalDate"
                    ]
                ),
                transitTime=int(
                    (
                        datetime.datetime.fromisoformat(inbound_eta) - datetime.datetime.fromisoformat(inbound_etd)
                    ).days
                ),
                transportations=Transportation.model_construct(
                    transportType=schedule_task["inboundInland"]["transMode"]
                ),
                voyages=Voyage.model_construct(internalVoyage="001"),
            )
        ]
        if schedule_task.get("inboundInland")
        else []
    )
    return leg_list
...
def process_schedule_data(
    task: dict, vessel_imo: str, service: str, tsp: str
) -> Iterator:
    check_service_code: bool = (
        any(
            services["vesselLoop"] == service
            for services in task["vessel"]
            if services.get("vesselDepartureDate")
        )
        if service
        else True
    )
    check_vessel_imo: bool = (
        any(imo for imo in task["vessel"] if imo.get("lloydRegisterNo") == vessel_imo)
        if vessel_imo
        else True
    )
    check_transshipment: str | None = task.get("transshipPortCode")
    transshipment_port_filter: bool = (
        bool(check_transshipment == tsp) if check_transshipment and tsp else False
    )
    if (
        (transshipment_port_filter or not tsp) and check_service_code and check_vessel_imo
    ):
        transit_time: int = task.get("totalTransitDay")
        first_pol: str = task.get("loadingPortCode")
        first_point_from: str = (
            task["outboundInland"]["fromUnLocationCode"]
            if task.get("outboundInland")
            else first_pol
        )
        last_point_to: str = (
            task["inboundInland"]["fromUnLocationCode"]
            if task.get("inboundInland")
            else task.get("dischargePortCode")
        )
        first_etd: str = task.get("departureDate")
        last_eta: str = task.get("arrivalDate")
        leg_list = process_leg_data(
            schedule_task=task,
...
async def get_hmm_p2p(
    ...
    params: dict = {
        ...
        "webSort": "D",
        "webPriority": (
            "D" if direct_only is True else "T" if direct_only is False else "A"
        ),
    }
    response_cache = await db.get(
        scac="hdmu",
        params=params,
        original_response=True,
        log_component="hdmu original response file",
    )
    ...
    def generate_schedule(data):
        return (
            ...
            for schedule_result in process_schedule_data(
                task=task, vessel_imo=vessel_imo, service=service, tsp=tsp
            )
        )
    if response_cache:
    ...
    response_json: dict = await anext(
        client.parse(scac="hmm", method="POST", url=url, headers=headers, json=params)
    )
    if response_json and response_json.get("resultMessage") == "Success":
        ...
        background_task.add_task(
            ...
            original_response=True,
            value=response_json,
            log_component="hmm original response file",
        )
        return p2p_schedule
