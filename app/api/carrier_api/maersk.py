...
def process_schedule_data(resp: dict,first_cut_off:dict, direct_only:bool |None,vessel_imo: str, service: str, tsp: str) -> Iterator:
...
async def get_cutoff_first_leg(client:HTTPClientWrapper,cut_off_url:str,cut_off_pw:str,response_data:list) -> dict:
    ...
    cut_off_leg: list = [asyncio.create_task(get_maersk_cutoff(client=client, url=cut_off_url, headers={'Consumer-Key': cut_off_pw}, country=leg.get('country'),pol=leg.get('pol'), imo=leg.get('imo'), voyage=leg.get('voyage')))
...
async def retrieve_geo_locations(client:HTTPClientWrapper, background_task:BackgroundTasks, pol:str, pod:str, location_url:str, pw:str):
    location_tasks = (asyncio.create_task(anext(client.parse(stream=True,background_tasks=background_task,method='GET',url=location_url,headers={'Consumer-Key': pw},
...
async def get_maersk_p2p(client:HTTPClientWrapper,background_task:BackgroundTasks,url: str, location_url: str, cutoff_url: str, pw: str, pw2: str, pol: str, pod: str,start_date: datetime.date,
    ...
    if origin_geo_location and des_geo_location:
        ...
        response_json: dict = await anext(client.parse(stream = True, background_tasks=background_task,method='GET', url=url, params=dict(params, **{'vesselOperatorCarrierCode': scac}),headers= headers,namespace=f'{scac} original response'))
        check_ocean_products = response_json.get('oceanProducts') if response_json else None
        if check_ocean_products:
            first_cut_off:dict = await get_cutoff_first_leg(client=client,cut_off_url=cutoff_url,cut_off_pw = pw,response_data=check_ocean_products)
            return (schedule_result for schedule in check_ocean_products for schedule_result in process_schedule_data(resp=schedule,first_cut_off=first_cut_off, direct_only=direct_only,vessel_imo=vessel_imo, service=service, tsp=tsp))
