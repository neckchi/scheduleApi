...
class ClientSideCache:
    ...
    async def preload_port_code_cache(self, extra: str | None = None):
        try:
            logging.info("Starting cache preload from Redis",
                         extra={'custom_attribute': None}) if extra else logging.info(
                "Starting cache preload from Redis")
            cursor: int = 0
            while True:
            ...
            logging.info(f"Cache preload completed. Loaded {len(self.port_mapping_cache)} entries.",
                         extra={'custom_attribute': None}) if extra else logging.info(
                f"Cache preload completed. Loaded {len(self.port_mapping_cache)} entries.")
    ...
    async def refresh_port_mapping_cache(self):
        ...
        return {
            f"Cleared port mapping cache from API hub. Items removed: {before_count}.Loaded the latest port code mapping into API hub"}
    ...
    async def read_port_mapping_code(self, scac: str | None = None, kn_port_code: str | None = None):
        ...
        try:
            while True:
                cursor, keys = await self._pool.scan(cursor=cursor, match=self.build_match_pattern(scac=scac,
                                                                                                   kn_port_code=kn_port_code),
                                                     count=100000)
                if cursor == 0:  # When cursor returns to 0, the scan is complete
                    break
            ...
            if keys:
                async with self._pool.pipeline(transaction=True) as pipe:
                    ...
                    for original_record, redis_value in zip(keys, redis_results):
                        ...
                        carrier_port_code: dict = {key.decode('utf-8'): value.decode('utf-8') for key, value in
                                                   redis_value.items()}
    ...
    async def set(self, key: str, value: JSONResponse,
                  ...
                  namespace: Optional[str] = 'data'):
        hashKey = self.generate_uuid_from_string(namespace=namespace, key=key)
        async with self._pool.pipeline(transaction=True) as pipe:
            try:
                await pipe.watch(
                    hashKey)  # tells Redis to monitor this key. If this key is modified by any other client before the transaction is executed, the transaction will be aborted.
                ...
                redis_set = pipe.set(name=hashKey, value=orjson.dumps(value), ex=expire,
