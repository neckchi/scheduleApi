...
async def get_maersk_cutoff(client, url: str, headers: dict, country: str, pol: str, imo: str, voyage: str):
    ...
    async for response_json in call_client(client=client,url=url,method ='GET',stream=True,headers=orjson.dumps(headers), params=orjson.dumps(params)):
        cut_off_body: dict = {}
...
async def get_maersk_p2p(client, url: str, location_url: str, cutoff_url: str, pw: str, pw2: str, pol: str, pod: str,
    ...
    location_tasks:Generator = (asyncio.create_task(anext(call_client(client=client,method='GET',stream=True, url=location_url, headers=orjson.dumps({'Consumer-Key': pw}), params= orjson.dumps({'UNLocationCode': port})))) for port in [pol, pod])
    ...
    async def schedules():
        if origingeolocation and destinationgeolocation:
            ...
            p2p_resp_tasks:list = [asyncio.create_task(anext(call_client(client=client, method='GET', url=url,params=orjson.dumps(dict(params, **{'vesselOperatorCarrierCode': mseries})),headers=orjson.dumps({'Consumer-Key': pw2})))) for mseries in maersk_list]
