    ...
    leg_list: list = [
        Leg.model_construct(
            pointFrom=PointBase.model_construct(
                locationName=leg["Calls"][0]["Name"],
                locationCode=leg["Calls"][0]["Code"],
                terminalName=leg["Calls"][0]["EHF"]["Description"],
                terminalCode=(
                    leg["Calls"][0]["DepartureEHFSMDGCode"]
                    if leg["Calls"][0]["DepartureEHFSMDGCode"] != ""
                    else None
                ),
            ),
            pointTo=PointBase.model_construct(
                locationName=leg["Calls"][-1]["Name"],
                locationCode=leg["Calls"][-1]["Code"],
                terminalName=leg["Calls"][-1]["EHF"]["Description"],
                terminalCode=(
                    leg["Calls"][-1]["ArrivalEHFSMDGCode"]
                    if leg["Calls"][-1]["ArrivalEHFSMDGCode"] != ""
                    else None
                ),
            ),
            etd=(
                etd := next(
                    led["CallDateTime"]
                    for led in leg["Calls"][0]["CallDates"]
                    if led["Type"] == "ETD"
                )
            ),
            eta=(
                eta := next(
                    lea["CallDateTime"]
                    for lea in leg["Calls"][-1]["CallDates"]
                    if lea["Type"] == "ETA"
                )
            ),
            transitTime=int(
                (datetime.fromisoformat(eta) - datetime.fromisoformat(etd)).days
            ),
            cutoffs=(
                Cutoff.model_construct(
                    docCutoffDate=si_cutoff,
                    cyCutoffDate=next(
                        (
                            led["CallDateTime"]
                            for led in leg["Calls"][0]["CallDates"]
                            if led.get("CallDateTime") and led["Type"] == "CYCUTOFF"
                        ),
                        None,
                    ),
                    vgmCutoffDate=next(
                        (
                            led["CallDateTime"]
                            for led in leg["Calls"][0]["CallDates"]
                            if led.get("CallDateTime") and led["Type"] == "VGM"
                        ),
                        None,
                    ),
                )
                if (
                    si_cutoff := next(
                        (
                            led["CallDateTime"]
                            for led in leg["Calls"][0]["CallDates"]
                            if led["Type"] == "SI" and led.get("CallDateTime")
                        ),
                        None,
                    )
                )
            ...
            transportations=Transportation.model_construct(
                ...
                referenceType=(
                    "IMO"
                    if (imo_code := leg.get("IMONumber")) and imo_code != ""
                    else None
                ),
                reference=imo_code if imo_code != "" else None,
            ),
            services=(
                Service.model_construct(serviceCode=leg["Service"]["Description"])
                if leg.get("Service")
                else None
            ),
            voyages=Voyage.model_construct(
                internalVoyage=(
                    leg["Voyages"][0]["Description"] if leg.get("Voyages") else None
                )
            ),
        )
        for leg in leg_task
...
def process_schedule_data(
    ...
    check_service_code: bool = (
        any(
            service_desc.get("Service") and service_desc["Service"]["Description"] == service
            for service_desc in task["Schedules"]
        )
        if service
        else True
    )
    ...
    transshipment_port = (
        any(tsport["Calls"][0]["Code"] == tsp for tsport in task["Schedules"][1:])
        if check_transshipment and tsp
        else False
    )
    check_vessel_imo: bool = (
        any(imo for imo in task["Schedules"] if imo.get("IMONumber") == vessel_imo)
        if vessel_imo
        else True
    )
    if (
        (transshipment_port or not tsp) and (direct_only is None or check_transshipment != direct_only) and check_service_code and check_vessel_imo
    ):
        first_point_from: str = task["Schedules"][0]["Calls"][0]["Code"]
        last_point_to: str = task["Schedules"][-1]["Calls"][-1]["Code"]
        first_etd: str = next(
            ed["CallDateTime"]
            for ed in task["Schedules"][0]["Calls"][0]["CallDates"]
            if ed["Type"] == "ETD"
        )
        last_eta: str = next(
            ed["CallDateTime"]
            for ed in task["Schedules"][-1]["Calls"][-1]["CallDates"]
            if ed["Type"] == "ETA"
        )
        transit_time: int = int(
            (datetime.fromisoformat(last_eta) - datetime.fromisoformat(first_etd)).days
        )
...
async def get_msc_token(
    ...
    if response_token is None:
        ...
        private_rsa_key = serialization.load_pem_private_key(
            b64decode(rsa), password=None, backend=default_backend()
        )
        ...
        response_token: dict = await anext(
            client.parse(
                ...
                headers=headers,
                data=params,
                token_key=msc_token_key,
                expire=timedelta(minutes=40),
            )
        )
    return response_token["access_token"]


async def get_msc_p2p(
    client: HTTPClientWrapper,
    background_task: BackgroundTasks,
    url: str,
    oauth: str,
    ...
    def generate_schedule(data):
        return (
            schedule_result
            for task in data["MSCSchedule"]["Transactions"]
    ...
    response_cache = await db.get(
        ...
        scac="mscu",
        params=str(params),
        log_component="msc original response",
    )
    if response_cache:
    ...
    response_json: dict = await anext(
        client.parse(scac="msc", method="GET", url=url, params=params, headers=headers)
    )
    if response_json:
        ...
        background_task.add_task(
            ...
            params=str(params),
            value=response_json,
            log_component="msc original response file",
        )
        return p2p_schedule
