...
from fastapi import status,HTTPException,BackgroundTasks,Response,Request
from fastapi.responses import JSONResponse
from fastapi.encoders import jsonable_encoder
from fastapi.exceptions import RequestValidationError, ResponseValidationError
from typing import Generator,Callable,AsyncGenerator,Dict,Any,Optional
...
class HTTPClientWrapper:
    ...
    async def parse(self,  url: str, method: str = 'GET', params: Optional[Dict[str, Any]] = None,
                    ...
                    expire: timedelta = timedelta(hours=load_yaml()['data']['backgroundTasks']['scheduleExpiry']),namespace:str|None = None,
                    stream: bool = False) -> AsyncGenerator[Dict[str, Any], None]:
        """Fetch the file from carrier API and deserialize the json file """
        if not stream:
            async for response in self.handle_standard_response(url, method, params, headers, json, data, background_tasks=background_tasks, expire=expire,namespace=namespace):
                yield response
        else:
            async for response in self.handle_streaming_response(url, method, params, headers, data, background_tasks=background_tasks, expire=expire,namespace=namespace):
                yield response


    async def handle_extra_response(self, url: str, method: str, params: Optional[Dict[str, Any]],headers: Optional[Dict[str, Any]],json: Optional[Dict[str, Any]], data: Optional[Dict[str, Any]],)-> AsyncGenerator[Dict[str, Any], None] :
        start_time = time.time()
        async with self._client.request(method=method, url=url, params=params, headers=headers,json=json, data=data, proxy=self.proxy) as extra_response:
            response_time = time.time() - start_time
            logging.info(f'{method} took {response_time:.2f}s to process the request {extra_response.url} {extra_response.status}')
            if extra_response.status in (status.HTTP_206_PARTIAL_CONTENT,status.HTTP_200_OK) and extra_response is not None:
    ...
    async def handle_standard_response(self,  url: str, method: str, params: Optional[Dict[str, Any]],
                                       ...
                                       background_tasks: Optional[BackgroundTasks], expire: timedelta,namespace:str|None = None) -> AsyncGenerator[Dict[str, Any], None]:
        try:
            ...
            if cache_result:
                ...
                async with self._client.request(method=method, url=url, params=params, headers=headers, json=json, data=data, proxy=self.proxy) as response:
                    response_time = time.time() - start_time
                    logging.info(f'{method} took {response_time:.2f}s to process the request {response.url} {response.status}')
                    if response.status == status.HTTP_206_PARTIAL_CONTENT:
                        ...
                        extra_p2p_task = [asyncio.create_task(anext(self.handle_extra_response(method="GET",url=url,params=params,json=json, data=data,headers=dict(headers, **{'range': f'{num}-{49 + num}'})))) for num in range(page, last_page, page)]
                        extra_p2p_result = await asyncio.gather(*extra_p2p_task)
                        combined_schedule.extend(*extra_p2p_result)
                        if background_tasks:
                            background_tasks.add_task(db.set, key=url + str(params), value=combined_schedule, expire=expire, namespace=namespace)
                        yield combined_schedule
                    elif response.status == status.HTTP_200_OK:
                        response_json = await response.json()
                        if background_tasks:
                            background_tasks.add_task(db.set, key=url + str(params), value=response_json, expire=expire, namespace=namespace)
                        yield response_json
    ...
    async def handle_streaming_response(self, url: str, method: str, params: Optional[Dict[str, Any]],
                                        ...
                                        expire: timedelta,namespace:str|None = None) -> AsyncGenerator[Dict[str, Any], None]:
        try:
            cache_result = await db.get(key=url + str(params),namespace=namespace) if namespace else None
            if cache_result:
                yield cache_result
                ...
                async with self._client.request(method, url=url, params=params, headers=headers, data=data, proxy=self.proxy) as stream_request:
                    ...
                    response_time = time.time() - start_time
                    logging.info(f'{method} took {response_time:.2f}s to process the request {stream_request.url} {stream_request.status}')
                    if stream_request.status == status.HTTP_200_OK:
                        async for data in stream_request.content:
                            response = orjson.loads(data)
                            if background_tasks:
                                background_tasks.add_task(db.set, key=url + str(params), value=response, expire=expire, namespace=namespace)
                            yield response
    ...
    def gen_all_valid_schedules(self,  request:Request, response: Response, product_id: UUID,
        ...
        response.headers.update({"Connection":"Keep-Alive", "Cache-Control": "max-age=7200,stale-while-revalidate=86400","KN-Count-Schedules": str(count_schedules)})
