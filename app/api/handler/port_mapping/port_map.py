from fastapi import APIRouter,UploadFile,HTTPException,status,Query,Depends
from fastapi.responses import JSONResponse
from fastapi.encoders import jsonable_encoder
from app.api.schemas.schema_request import PortCodeMapping,PORT_CODE_ADAPTER,CarrierCode
...
async def upload_port_code_mapping(upload_file:UploadFile):
    ...
    try:
        ...
        if reader.fieldnames is required_columns:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="File is missing one or more required columns.")
        try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"File upload and validation failed: {str(e)}")

@router.get("/read", summary="Read port code mapping information",response_model=List[PortCodeMapping])
async def read_port_code_mapping(scac: CarrierCode|None = Query(default=None,description='Search for port code by carrier code'),
                                 kn_port_code: str|None = Query(alias='knPortCode',default=None,max_length=5,pattern=r"[A-Z]{2}[A-Z0-9]{3}",example='HKHKG',description='Search by either port or point of origin')):
    try:
...
async def refresh_port_code_mapping(query_params: Annotated[PortCodeMapping, Query()]):
    ...
    """
    try:
        updated_result = await db.update_carrier_port_code(scac=query_params.scac,kn_port_code=query_params.kn_port_code,new_carrier_port_code=query_params.carrier_port_code)
...
async def delete_port_code_mapping(scac: CarrierCode|None = Query(default=None,description='Search for port code by carrier code'),
                                 kn_port_code: str|None = Query(alias='knPortCode',default=None,max_length=5,pattern=r"[A-Z]{2}[A-Z0-9]{3}",example='HKHKG',description='Search by either port or point of origin')):
    """
