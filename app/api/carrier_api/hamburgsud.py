...
async def get_sudu_p2p(client, url: str, pw: str, pol: str, pod: str, direct_only: bool | None = None,
                       tsp: str | None = None,
	...
	async def schedules():
		...
		if response.status_code == 200:
			...
			for task in response_json:
				...
				transshipment_port: bool = next(
					(True for tsport in task['leg'][1:] if tsport['from']['unlocode'] == tsp),
					False) if check_transshipment and tsp else False
				if transshipment_port or not tsp:
					...
					first_cy_cutoff: str = next(
						(cyc['cargoCutOffLT'] for cyc in task['leg'] if cyc['cargoCutOffLT'] is not None), None)
						...
						for legs in task['leg']:
							...
							leg_body: dict = {'pointFrom': {'locationCode': legs['from']['unlocode'],
							                  ...
							                  'transitTime': int((datetime.fromisoformat(
								                  legs['expectedArrivalLT'][:10]) - datetime.fromisoformat(
