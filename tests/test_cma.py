...
config_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'app', 'internal', 'setting.py'))
...
from app.api.handler.p2p_schedule.carrier_api.cma import get_cma_p2p, process_schedule_data, process_leg_data, \
    DEFAULT_ETD_ETA
    ...
    async def test_get_cma_p2p_no_results(self, mock_process_schedule_data):
        ...
        with patch('app.storage.db.get', new_callable=AsyncMock) as mock_db_get, \
          patch('app.api.handler.p2p_schedule.carrier_api.cma.fetch_schedules', new_callable=AsyncMock) as mock_fetch:
            ...
            result = await get_cma_p2p(client=mock_client, background_task=mock_background_task, api_settings=mock_settings, pol=pol, pod=pod, search_range=search_range, direct_only=direct_only)
            assert isinstance(result, GeneratorType)
            result_list = list(result)
            assert len(result_list) == 0
    ...
    async def test_get_cma_p2p_cache_miss(self, mock_process_schedule_data):
        ...
        with patch('app.storage.db.get', new_callable=AsyncMock) as mock_db_get, \
          patch('app.api.handler.p2p_schedule.carrier_api.cma.fetch_schedules', new_callable=AsyncMock) as mock_fetch:
            ...
            mock_fetch.return_value = mock_response
            # Call the function
            result = await get_cma_p2p(client=mock_client, background_task=mock_background_task,api_settings=mock_settings,pol=pod, pod=pod, search_range=search_range, direct_only=direct_only,
                                       scac=scac)
