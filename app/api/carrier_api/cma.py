...
from app.api.schemas.schema_response import Schedule,Leg,PointBase,Voyage,Cutoff,Service,Transportation
...
def extract_transportation(transportation:dict):
    ...
    if vessel_imo and len(vessel_imo) < 9:
        ...
        reference_type:str = 'IMO'
        reference:str = '9'
    return Transportation.model_construct(transportType = mean_of_transport,transportName = vehicule.get('vehiculeName'),referenceType = reference_type,reference = reference)

def process_leg_data(leg_task:list)->list:
    leg_list: list = [Leg.model_construct(pointFrom = PointBase.model_construct(locationName = leg['pointFrom']['location']['name'],
        ...
        services = Service.model_construct(serviceCode = service_name) if (service_name := deepget(leg['transportation'], 'voyage', 'service', 'code')) else None,
        voyages = Voyage.model_construct(internalVoyage = voyage_num if (voyage_num := deepget(leg['transportation'], 'voyage', 'voyageReference')) else None),
        cutoffs = Cutoff.model_construct(docCutoffDate = deepget(leg['pointFrom']['cutOff'], 'shippingInstructionAcceptance', 'utc'),
                                         cyCutoffDate = deepget(leg['pointFrom']['cutOff'], 'portCutoff', 'utc'),
                                         vgmCutoffDate = deepget(leg['pointFrom']['cutOff'], 'vgm', 'utc')) if leg['pointFrom'].get('cutOff') else None) for leg in leg_task]
    return leg_list
...
def process_schedule_data(task: dict,direct_only:bool |None,service_filter:str|None,vessel_imo_filter:str|None) -> Iterator:
    ...
    first_etd = next((ed['pointFrom']['departureDateGmt'] for ed in task['routingDetails'] if ed['pointFrom'].get('departureDateGmt')), DEFAULT_ETD_ETA)
    last_eta = next((ea['pointTo']['arrivalDateGmt'] for ea in task['routingDetails'][::-1] if ea['pointTo'].get('arrivalDateGmt')), DEFAULT_ETD_ETA)
    check_transshipment:bool = len(task['routingDetails']) > 1
    check_service_code: bool = any(leg for leg in task['routingDetails'] if deepget(leg['transportation'], 'voyage', 'service', 'code') == service_filter) if service_filter else True
    check_vessel_imo: bool = any(leg for leg in task['routingDetails'] if deepget(leg['transportation'],'vehicule','reference') == vessel_imo_filter) if vessel_imo_filter else True
...
async def fetch_schedules(client: HTTPClientWrapper,background_task:BackgroundTasks, cma_code: str, url: str, headers: dict, params: dict, extra_condition: bool) -> dict:
    """Fetch the initial set of schedules from CMA."""
    updated_params = lambda cma_internal_code: {**params,**({'shippingCompany': cma_internal_code} if cma_internal_code is not None else {}),'specificRoutings': 'USGovernment' if cma_internal_code == '0015' and extra_condition else 'Commercial'}
    response_json: dict = await anext(client.parse(background_tasks=background_task, method='GET', url=url, params=updated_params(cma_code), headers=headers,namespace=f'{CMA_GROUP.get(cma_code) if cma_code else "CMDU"} original response'))
    if response_json:
        return response_json

async def get_cma_p2p(client:HTTPClientWrapper,background_task:BackgroundTasks,url: str, pw: str, pol: str, pod: str, search_range: int, direct_only: bool | None,tsp: str | None = None,vessel_imo:str | None = None,
                          departure_date: datetime.date = None,arrival_date: datetime.date = None, scac: str | None = None, service: str | None = None) -> Generator:
    generate_schedule = lambda data, direct_only, service, vessel_imo: (schedule_result for task in data for schedule_result in process_schedule_data(task=task, direct_only=direct_only, service_filter=service, vessel_imo_filter=vessel_imo))
    ...
    response_json = await fetch_schedules(client=client,background_task=background_task,url=url,headers=headers,params=params,cma_code=api_carrier_code,extra_condition=extra_condition)
    if response_json:
