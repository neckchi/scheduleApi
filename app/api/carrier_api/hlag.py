...
from app.api.schemas.schema_response import Schedule,Leg,PointBase,Voyage,Service,Transportation
...
def process_schedule_data(task: dict, service: str, tsp: str,vessel_imo:str) -> Iterator:
    ...
    check_service_code: bool = any(loop_code for loop_code in task['legs'] if loop_code.get('carrierServiceCode') == service) if service else True
    check_transshipment: bool = len(task['legs']) > 1
    check_vessel_imo: bool = any(imo for imo in task['legs'] if imo.get('vesselIMONumber') == vessel_imo) if vessel_imo else True
...
async def get_hlag_p2p(client:HTTPClientWrapper,background_task:BackgroundTasks,url: str, client_id: str,client_secret:str,pol: str, pod: str,search_range: int,
                       etd: datetime.date = None, eta: datetime.date = None, direct_only: bool|None = None,service: str | None = None, tsp: str | None = None,vessel_imo:str|None = None):
    ...
    generate_schedule = lambda data: (result for task in data for result in process_schedule_data(task=task, service=service, tsp=tsp,vessel_imo=vessel_imo))
    headers: dict = {'X-IBM-Client-Id': client_id, 'X-IBM-Client-Secret': client_secret, 'Accept': 'application/json'}
    response_json = await anext(client.parse(method='GET',background_tasks=background_task, url=url, params=params,headers=headers,namespace='hlag original response'))
    if response_json:
