...
async def get_msc_token(client:HTTPXClientWrapper,background_task:BackgroundTasks, oauth: str, aud: str, rsa: str, msc_client: str, msc_scope: str, msc_thumbprint: str):
    ...
    if response_token is None:
        ...
        response_token = await anext(client.parse(background_tasks =background_task,method='POST',url=oauth, headers=headers, data=params,token_key=msc_token_key,expire = timedelta(minutes=40)))
...
async def get_msc_p2p(client:HTTPXClientWrapper, background_task:BackgroundTasks,url: str, oauth: str, aud: str, pw: str, msc_client: str, msc_scope: str,msc_thumbprint: str, pol: str, pod: str,
    ...
    msc_response_uuid = uuid5(NAMESPACE_DNS,str(params)+str(direct_only)+str(vessel_imo)+str(service)+str(tsp))
    ...
    if response_cache is None:
        ...
        if response_json:
            ...
            for task in response_json['MSCSchedule']['Transactions']:
                ...
                if (transshipment_port or not tsp) and (direct_only is None or check_transshipment != direct_only) and check_service_code and check_vessel_imo:
                    ...
                    leg_list: list = [schema_response.Leg.model_construct(
                        ...
                        cutoffs={'docCutoffDate':si_cutoff,'cyCutoffDate': next((led['CallDateTime'] for led in leg['Calls'][0]['CallDates'] if  led.get('CallDateTime') and led['Type'] == 'CYCUTOFF'), None),
            ...
            background_task.add_task(db.set, key=msc_response_uuid, value=total_schedule_list) if total_schedule_list else ...
            return total_schedule_list
    else:return response_cache